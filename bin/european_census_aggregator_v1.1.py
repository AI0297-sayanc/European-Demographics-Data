...
2. Ensure that you have the necessary input files:
    - 'file1.json': Path to the file containing the list of census documents within the radius. (levelCode:3)

3. Run the code using the command-line interface.

    $ python european_census_aggregator.py <file1.json>
    

Update Log:
-----------

Version 1.1 (14-Mar-2024):
	- Fixed calculation for EU_E006 to ensure accurate results. Since the original calculation method for EU_E006 was found to produce inaccurate results.
	
Version 1.0 (12-Mar-2024):
	- Initial release of the European census aggregation function.



Last updated on: 14-Mar-2024 (v1.1)
...
def census_aggregator(census_collection_lst):
    ...
    for cntr_lvl in cntr_lvl_combinations:
        ...
        total_owned_hh = np.nansum(owned_households_arr)
        owned_households_perc = total_owned_hh / total_households
        ...
        total_rented_hh = np.nansum(rental_households_arr)
        rental_households_perc = total_rented_hh / total_households
    ...
    # sort output list based on keys:
    result_list = sorted(result_list, key=lambda x: (x['countryCode'], x['levelCode']))
